...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    HWND hwnd = CreateWindowEx(0, CLASS_NAME, "CSGROS Retro 3D", WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, SCREEN_WIDTH, SCREEN_HEIGHT, NULL, NULL, hInstance, NULL);
    ...
    while (running) {
        ...
        if (window_focused && (GetAsyncKeyState(VK_F11) & 0x8000)) {
            if (GetTickCount() - last_f11_time > 500) { // Anti-spam de la touche
        ...
        if (!p1.is_alive) {
            if (window_focused && (GetAsyncKeyState('R') & 0x8000)) init_player(&p1, p1.id, 2.0f, 2.0f);
        }

        bool shooting_now = false;
        
        if (p1.is_alive && window_focused) {
            // SOURIS : Calcul dynamique du centre selon la taille de la fenêtre (Fenêtré ou Plein Écran)
            RECT clRect;
                ...
                if (is_client_connected[i]) {
                    ...
                    for (int j = 2; j <= 4; j++) {
                        if (i != j && is_client_connected[j]) {
                            PlayerPacket relay_pkt = { j, Packets_players[j].x, Packets_players[j].y, Packets_players[j].angle, Packets_players[j].hp, Packets_players[j].is_shooting };
        ...
        // ÉTIREMENT DYNAMIQUE (Garde l'aspect Pixel-Art même en 1080p ou 4K !)
        RECT clRect;
        GetClientRect(hwnd, &clRect);
        int win_w = clRect.right - clRect.left;
        int win_h = clRect.bottom - clRect.top;

        SetStretchBltMode(hdc, COLORONCOLOR); // Empêche l'image de devenir floue au redimensionnement
